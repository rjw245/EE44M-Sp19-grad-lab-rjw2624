...
void ST7735_OutUDec(uint32_t n)
{
  ...
  ST7735_DrawString(StX, StY, Message, StTextColor, StTextColor);
...
void ST7735_OutChar(char ch)
{
  if ((ch == 10) || (ch == 13) || (ch == 27))
  {
    ...
    ST7735_DrawString(0, StY, "                     ", StTextColor, StTextColor);
    return;
  }
...
{
  int row = device * 8 + line * 2;
  uint32_t len = ST7735_DrawString(0, row, string, ST7735_BLACK, ST7735_WHITE);
  char value_str[12];
  sprintf(value_str, "%d    ", value);
  ST7735_DrawString(len, row, value_str, ST7735_BLACK, ST7735_WHITE);
}
