...
uint8_t static RCVData[32][4];
uint8_t static RCVARR[32];
...
void CAN0_Handler(void){ uint8_t data[4];
  ...
  if(ulIntStatus & CAN_INT_INTID_STATUS){  // receive?
    ...
    for(i = 0; i < 32; i++){    //test every bit of the mask  --> 32 possible inputs??
      if( (0x1 << i) & ulIDStatus){  // if active, get data
        CANMessageGet(CAN0_BASE, (i+1), &xTempMsgObject, true);
        for(j=0;j<32;j++)
        {
          int k = RCVARR[(j)%32];
          if(xTempMsgObject.ulMsgID == k){
            RCVData[k][0] = data[0];
            RCVData[k][1] = data[1];
            RCVData[k][2] = data[2];
            RCVData[k][3] = data[3];
...
void CAN0_GetMailwithIdx(uint8_t data[4], int idx){
  ...
  data[2] = RCVData[idx][2];
  data[3] = RCVData[idx][3];
}
